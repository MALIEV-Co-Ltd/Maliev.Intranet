...
- IAM role: `roles.aspire.automation`, with wildcard `*` permission for local validation. This role is intentionally separate from `roles.platform.owner` so it can be removed or disabled without changing real human owner access.

Do not use `system@maliev.com` for browser automation. That address is the IAM system/service principal, not a human employee login. Reusing it would blur audit trails, conflict with the existing system principal email, and risk making service-owned roles look like real human Platform Owner bootstrap state. Do not assign the automation principal to `roles.platform.owner`; it should use only the dedicated automation role.
...
6. If login fails:
   ...
   - Check that IAM binds the synthetic principal to `roles.aspire.automation` and that role contains `*`.
...
- The automation principal must not have `roles.platform.owner`; the Aspire IAM seeder removes that legacy binding for the synthetic principal.
