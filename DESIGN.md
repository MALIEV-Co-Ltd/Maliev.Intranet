...
This document is the visual inspiration for the Intranet implementation, not a literal marketing-page transplant. The implemented app preserves operational density, existing Blazor/MudBlazor workflows, and dark-mode support. Typography uses Geist with Noto Sans Thai fallback, but letter spacing remains 0 in implementation to match the application rules and avoid compressed operational text.
