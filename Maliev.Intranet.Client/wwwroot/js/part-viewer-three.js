 ...
 * converted to GLB server-side before reaching this script.
 *
...
const CONFIG = {
    CAMERA_FOV_DEG: 0.8 * (180 / Math.PI), // matches former BabylonJS FOV (0.8 rad)
    CAMERA_NEAR: 0.1,
    ...
    WIREFRAME_COLOR_LIGHT: 0x1f2226,
    WIREFRAME_COLOR_DARK: 0xcdd3de,

    GRID: { size: 10, divisions: 10 }, // multiplied by model radius at runtime
...
const modelRoots = {};
const fitRadiusMap = {};
const orthoBaseMap = {};       // { left, right, top, bottom } at zoom=1 (orthographic)
const cameraProjection = {};   // 'perspective' | 'orthographic'
const darkModes = {};
const lightsMap = {};          // canvasId -> { key, fill, rim, back, ambient }
const renderModes = {};        // 'solid' | 'wireframe' | 'transparent'
const edgesEnabled = {};
const bboxEnabled = {};
const gridEnabled = {};
const bboxHelpers = {};
const gridHelpers = {};
const edgeLinesByMesh = {};    // canvasId -> Map<mesh.uuid, LineSegments2>
const cadMeshes = {};          // canvasId -> Mesh[] (model meshes eligible for edges/bbox/material)
const bodyMaps = {};           // canvasId -> Map<index, { root, meshes, color, name }>
const selectedBody = {};
const bodyPickHandlers = {};
const dotNetRefs = {};
const loadGenerations = {};
const animStates = {};         // 'idle' | 'hovering' | 'interacting'
...
// Turning-axis overlay
const turningAxisObjects = {}; // canvasId -> { line, labelDiv }

// DFM overlays
const dfmOverlays = {};        // canvasId -> Map<"partKey|overlayKey", Object3D>

// Measure tool
const measureStates = {};      // canvasId -> { points: Vector3[], line, markers, clickHandler, dotNetRef }

// Thickness analysis
const thicknessStates = {};    // canvasId -> { markers: Object3D[], clickHandler }

// Flipped-triangle view
const flippedTriangleOriginalMaterials = {}; // canvasId -> Map<mesh.uuid, material>

// Cutting mat
const cuttingMatMeshes = {};   // canvasId -> Object3D
...
function debugLog(...args) {
    if (typeof window !== 'undefined' && window.__partViewerDebug) {
...
function measureFpsAndAdaptPixelRatio(canvasId, timestamp) {
    ...
    lod.lastT = timestamp;
    if (dt <= 0 || dt > 200) return; // first frame or gap after idle — skip noisy sample
    lod.samples[lod.fpsIdx] = dt;
    lod.fpsIdx = (lod.fpsIdx + 1) % LOD_FPS_SAMPLE_SIZE;
    if (lod.fpsCnt < LOD_FPS_SAMPLE_SIZE) lod.fpsCnt++;
    if (lod.fpsCnt < 5) return; // need at least 5 samples before acting
    let sum = 0;
    ...
    if (next !== level) {
        ...
        lod.fpsCnt = 0; // reset sample window after each quality change
    }
}
...
/** Recursively collects "body" root nodes the same way the BabylonJS viewer did:
 *  unwrap single-child wrapper nodes, treat siblings as distinct bodies. */
...
// ============================================================================
// CAMERA PRESETS (quaternion arc animation — mirrors the former ArcRotateCamera math)
// ============================================================================
...
/**
 * Animates the camera from its current position/up to a preset direction/up,
 * using quaternion SLERP so the transition is stable at any angle (incl. 180°).
 */
...
function createOrbitControls(canvasId, camera, canvas, fitRadius) {
    ...
    controls.enablePan = false; // panning is handled by the custom right-click handler below
    ...
    controls.addEventListener('start', () => {
        ...
        // Hide fat-line edges during active drag for better interactive frame rate.
        if (edgesEnabled[canvasId] && renderModes[canvasId] !== 'wireframe') {
            setEdgeLinesVisible(canvasId, false);
        }
    });
    controls.addEventListener('end', () => {
        // Restore edges once the user releases the mouse.
        if (edgesEnabled[canvasId] && renderModes[canvasId] !== 'wireframe') {
...
/** Right-click drag pans the camera target so the world point under the
 *  cursor at mousedown stays under the cursor — same algorithm as the
 *  former BabylonJS implementation (ray/plane intersection). */
...
function makeXrayMaterial(spec) {
    return new THREE.MeshStandardMaterial({
        color: spec.color, transparent: true, opacity: spec.opacity, depthWrite: false, side: THREE.DoubleSide,
    });
}
...
export function showGrid(canvasId) {
    ...
    if (!gridHelpers[canvasId]) {
        ...
        helper.rotation.x = Math.PI / 2; // grid is XZ by default; rotate into the XY ground plane (Z-up)
...
export function setBodies(canvasId, _bodies) {
    ...
    // this hook exists for parity with the Blazor JSInterop contract.
}
...
export async function initialize(canvasId, fileUrl, fileExt, isDark, knownDimsMm, dotNetRef, viewerSettings = {}) {
    ...
    try {
        ...
        if (loadGenerations[canvasId] !== generation) return; // superseded by a newer initialize()
        ...
        // ── Scale correction against known dimensions (server-reported mm bbox) ──
        ...
        // ── Multi-body detection + baseline CAD-gray material ──
...
function startRenderLoop(canvasId, generation) {
    ...
    // Initialise per-canvas LOD tracker if not already present.
    ...
    function frame(timestamp) {
        ...
        // Step auto-rotation. If the camera actually moved, mark the frame dirty.
        ...
        if (Math.abs(autoSpeedCurrent[canvasId]) > CONFIG.AUTO_ROTATION_MIN_THRESHOLD) {
            ...
            // Auto-rotation moves the camera every frame — keep frames dirty.
            const lod = renderLod[canvasId];
            if (lod) lod.frames = 2;
        }

        // controls.update() with enableDamping=false is cheap and may emit 'change'
        // (which calls markDirty) when OrbitControls applies pointer input.
        controls.update();

        // On-demand rendering: only render when the scene has changed.
        const lod = renderLod[canvasId];
        if (lod && lod.frames > 0) {
            lod.frames--;
...
export function setSectionPlane(canvasId, enabled, axis, offsetMm, inverted) {
    ...
    // Simple translucent cap so the cut isn't a hollow see-through hole.
    if (box) {
...
export function clearTurningAxis(canvasId) {
...
export function toggleLocalDfmOverlay(canvasId, partKey, overlayKey, faceIndices, visible) {
    ...
    const existing = map.get(key);
    if (existing) { scene.remove(existing); disposeObject3D(existing); map.delete(key); }
    if (!visible || !faceIndices || faceIndices.length === 0) return;

    const sourceMesh = (cadMeshes[canvasId] || [])[0];
    if (!sourceMesh) return;
    const sourcePos = sourceMesh.geometry.attributes.position;
    const sourceIndex = sourceMesh.geometry.index;
    const positions = [];
    faceIndices.forEach((triIndex) => {
        for (let v = 0; v < 3; v += 1) {
            const vi = sourceIndex ? sourceIndex.getX(triIndex * 3 + v) : triIndex * 3 + v;
            positions.push(sourcePos.getX(vi), sourcePos.getY(vi), sourcePos.getZ(vi));
        }
    });
...
function makeMeasureMarker(point) {
    ...
    const marker = new THREE.Mesh(
        new THREE.SphereGeometry(Math.max(fit * CONFIG.MEASURE.endpointRadiusRatio, 0.3), 12, 12),
...
// ============================================================================
// CUTTING MAT (simplified: flat green grid floor, no logo/labels/animation)
// ============================================================================

function buildCuttingMatTexture() {
    const size = 512;
...
export function showCuttingMat(canvasId) {
    ...
    const matW = Math.max(300, size.x + 120);
    const matH = Math.max(220, size.y + 120);
    const geometry = new THREE.PlaneGeometry(matW, matH, Math.round(matW / 10), Math.round(matH / 10));
    const material = new THREE.MeshStandardMaterial({ map: buildCuttingMatTexture(), roughness: 0.85 });
    const mesh = new THREE.Mesh(geometry, material);
    mesh.position.z = box.min.z;
    mesh.receiveShadow = false;
    scene.add(mesh);
    cuttingMatMeshes[canvasId] = mesh;
    markDirty(canvasId);
}

export function hideCuttingMat(canvasId) {
    const scene = scenes[canvasId];
    const mesh = cuttingMatMeshes[canvasId];
    if (scene && mesh) { scene.remove(mesh); disposeObject3D(mesh); }
    delete cuttingMatMeshes[canvasId];
    markDirty(canvasId);
}
