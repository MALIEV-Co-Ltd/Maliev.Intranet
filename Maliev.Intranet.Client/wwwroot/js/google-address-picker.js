(function () {
    ...
    async function initializeSearch(elementId, dotNetReference, config) {
        ...
        const handler = async event => {
            ...
            const selection = normalizeSelection("GooglePlace", place, place.location, place.addressComponents);
            await dotNetReference.invokeMethodAsync("NotifyGoogleAddressSelected", selection);
        };
    ...
    async function initializeMap(elementId, dotNetReference, config, current) {
        ...
        const publishLocation = async latLng => {
            ...
            const selection = normalizeSelection("GoogleMapPin", result, literal, result.address_components);
            await dotNetReference.invokeMethodAsync("NotifyGoogleAddressSelected", selection);
        };
